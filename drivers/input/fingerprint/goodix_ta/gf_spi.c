...
static inline int gf_power_switch(struct gf_dev *gf_dev, int status)
{
	if (gpio_is_valid(gf_dev->pwr_gpio)) {
		gpio_direction_output(gf_dev->pwr_gpio, status);
		if (status)
...
static inline int gf_probe(struct platform_device *pdev)
{
	...
	mutex_lock(&gf_lock);
	device_create(gf_class, &gf_dev->spi->dev, gf_dev->devt, gf_dev, GF_DEV_NAME);
...
{
	int status = register_chrdev(SPIDEV_MAJOR, "goodix_fp_spi", &gf_fops);
	SPIDEV_MAJOR = status;
